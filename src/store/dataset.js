...
export const useDatasetStore = defineStore({
  ...
  actions: {
    ...
    async prepareDataset(projectId) {
      return await this.$fs.createDir(projectId || this.project)
    },
    ...
    async addData(data) {
      ...
      if (data.mfcc && data.mfcc_ext) {
        await this.$fs.writeFile(`${this.project}/${data.id}_mfcc.${data.mfcc_ext}`, data.mfcc)
      }
    ...
    async deleteDatasetItem({ id, ext }) {
      ...
      await this.$fs.removeFile(`${this.project}/${id}.${ext}`)
    ...
    async deleteDatasetItems(ids) {
      ...
      for (let item of selectedItems) {
        await this.$fs.removeFile(`${this.project}/${item.id}.${item.ext}`)
      }
    },
    ...
    async getDataAsFile(filename) {
      return await this.$fs.readAsFile(
        `${this.project}/${filename}`,
      )
    },
