  ...
  >
    <VCard class="pa-sm-3 pa-3 bg-background">
      ...
      <VCardItem>        
        ...
        <VTabs v-model="tab" direction="vertical" color="primary">
          <VTab value="Installed">
            <VIcon class="mr-2">mdi-download-box</VIcon>
            Installed Plugins
          </VTab>
          <VDivider class="my-1"></VDivider>
          <VTab v-for="cat in categories" :key="cat.value" :value="cat.value">
            <VIcon class="mr-2">{{ cat.icon }}</VIcon>
            {{ cat.title }}
          </VTab>
        </VTabs>
        <VWindow v-model="tab" class="window-tab">
          <VWindowItem value="Installed">
            <VRow class="px-2">
              <VCol v-for="plugin in installedPlugins" cols="12" md="4">
                <VCard color="grey-lighten-4">
                  <VToolbar dark density="compact">
                    <VIcon class="mx-2">mdi-puzzle</VIcon>
                    <div>{{ plugin.name }}</div>
                    <VSpacer></VSpacer>  
                    <div class="me-3">Version : {{ plugin.version }}</div>
                  </VToolbar>
                  <VCardText class="d-flex align-center">
                    <VImg :src="plugin.path +'/'+plugin.icon" width="100" height="100" class="mx-2"></VImg>
                  </VCardText>
                  <VCardText>
                    <div class="d-flex flex-row">
                      ...
                      <v-spacer></v-spacer>
                      <v-btn v-if="!isInstalled(plugin)" icon color="primary" @click="emit('installPlugin',plugin)">
                        <VProgressCircular v-if="plugin.installing" indeterminate color="white" width="3" size="32"></VProgressCircular>
                        <v-icon v-else>mdi-download</v-icon>
                      </v-btn>
                      <v-btn v-else icon color="error" @click="emit('uninstallPlugin',plugin)">
                        <VProgressCircular v-if="plugin.installing" indeterminate color="white" width="3" size="32"></VProgressCircular>
          ...
          <VWindowItem v-for="cat in categories" :key="cat.value" :value="cat.value">
            <VRow class="px-2">
              <VCol v-for="plugin in listPluginByCategory(cat.value)" cols="12" md="4">
                <VCard color="grey-lighten-4">
                  <VToolbar dark density="compact">
                    <VIcon class="mx-2">mdi-puzzle</VIcon>
                    <div>{{ plugin.name }}</div>
