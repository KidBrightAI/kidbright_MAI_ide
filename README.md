...
Current release: **1.2.4** (2026-08-08). Live at <https://kidbright-mai.web.app>.

### 1.2.4 — บล็อกควบคุมไฟบนบอร์ด mAI Plus และหลอด RGB ภายนอก

**Added**
- **บล็อกควบคุมไฟบนบอร์ด** สั่งเปิด/ปิดไฟดวงบนบอร์ด mAI Plus ได้จากโปรแกรมของผู้ใช้ ไฟดวงนี้เป็นอุปกรณ์ชนิด `leds-gpio` ซึ่งไดรเวอร์ของเคอร์เนลเป็นเจ้าของขาสัญญาณ จึงควบคุมผ่าน `/sys/class/leds/led-user/brightness` แทนการเรียก `maix.gpio` โดยตรง generator จะปิด kernel trigger เดิม (ค่าโรงงานคือ `activity`) หนึ่งครั้งก่อนเขียนค่า และบีบค่าที่ผู้ใช้ใส่ให้อยู่ในช่วง 0-255 เสมอ · เป็นไฟติด/ดับตามฮาร์ดแวร์ ไม่ใช่ไฟหรี่ระดับได้

**Fixed**
- **บล็อก RGB LED สร้างโค้ดไม่ได้มาตั้งแต่ต้น** ทั้ง `maixpy3_gpio_rgb` และ `maixpy3_gpio_rgb_hex` มีเพียงนิยามบล็อก ส่วน generator ยังเป็น `TODO` ที่คืนค่าสตริง `'...'` ซึ่งรันไม่ได้ บล็อกทั้งสองจึงถูกปิดไม่ให้แสดงในกล่องเครื่องมือ ตอนนี้ generator สร้างโค้ด Python ที่ใช้งานได้จริงแล้ว และเปิดบล็อกกลับเข้ากล่องเครื่องมือ พร้อมเพิ่มช่องให้ผู้ใช้เลือกขาที่ต่อหลอดแต่ละสีได้เอง (ค่าเริ่มต้น R=IO2, G=IO3, B=IO4)

  หลอดภายนอกสั่งงานแบบติด/ดับต่อสี (ค่าตั้งแต่ 128 ขึ้นไปถือว่าติด) ไม่ได้ไล่ระดับความสว่างด้วย PWM เพราะในชุดขาที่ผู้ใช้ต่อได้ (IO2-IO8) มีเพียง IO7 ที่ต่อกับ PWM ของชิป ส่วนช่อง PWM อื่นถูกใช้โดยพอร์ตคอนโซลและช่อง SD card ไปแล้ว ขณะที่หลอด RGB ต้องใช้สามขาพร้อมกัน

  ทดสอบบนบอร์ดจริง: ไฟบนบอร์ดสั่ง 255/0/128 แล้วอ่านค่ากลับได้ตรงทุกค่า ค่าเกินขอบเขต 999 และ -5 ถูกบีบเป็น 255 และ 0 ตามลำดับ · หลอดภายนอกสั่งสีแล้วอ่านสถานะขาสัญญาณกลับมาตรงทุกครั้ง และใช้สามบล็อกร่วมกันในโปรแกรมเดียวได้โดยไม่สร้างตัวแปรซ้ำ · ยังไม่ได้ทดสอบกับหลอด RGB จริงเพราะไม่มีอุปกรณ์หลอดมาต่อ

### 1.2.3 — บอร์ด mAI Plus ตอบผลของสิ่งที่เพิ่งป้อนเข้าไป ไม่ใช่ของครั้งก่อน

**Fixed**
- **โมเดลบนบอร์ด V2 ตอบผลของการเรียกครั้งก่อนหน้า และการเรียกครั้งแรกได้ค่าว่าง** ทั้งสาม runtime (`voice_runtime`, `classifier_runtime`, `detector_runtime`) สร้างโมเดลด้วย `dual_buff=True` ซึ่งทำ pipelining บน NPU อาการนี้มองไม่เห็นในลูปกล้องที่ประมวลผลต่อเนื่อง เพราะช้าไปหนึ่งเฟรมไม่มีใครสังเกต แต่พังสนิทกับการใช้งานแบบครั้งเดียวจบ เช่น พูดหนึ่งคำแล้วรอผล หรือกดถ่ายรูปแล้วให้ทาย ซึ่งจะได้คำตอบของสิ่งที่ทำครั้งที่แล้ว จากมุมผู้ใช้มันดูเหมือนโมเดลทายมั่วทั้งที่โมเดลถูกต้อง

  วัดบนบอร์ดจริง: เสียง 30 คลิป เปิดโหมดนี้ได้ "ถูก" 29/30 แต่คำตอบตรงกับคลิปก่อนหน้า 29 จาก 29 ครั้งและคลิปแรกความมั่นใจ 0.00 · ปิดแล้วได้ 30/30 · ภาพสลับสองแบบ เปิดได้ผลของภาพก่อนหน้าตลอด ปิดแล้วตรงกับภาพที่ป้อนทุกครั้ง · ตรวจจับวัตถุ เปิดแล้วครั้งแรกไม่เจออะไรทั้งที่ในภาพมีวัตถุ ปิดแล้วเจอตั้งแต่ครั้งแรก

  การปิดโหมดนี้ไม่ได้ทำให้ช้าลง วัดได้ 11.07 ms ต่อครั้ง (90 fps) เทียบกับ 12.96 ms (77 fps) เพราะจุดที่เรียกใช้ล้วนรอผลทันทีอยู่แล้ว ไม่มีงานอื่นให้ทำคาบเกี่ยว

### 1.2.2 — Voice classification บนบอร์ด mAI Plus ใช้งานได้จริง

**Fixed**
- **โมเดลเสียงบนบอร์ด V2 ทายมั่วเหมือนเดาสุ่ม** เกิดจากภาพ spectrogram ที่ใช้ตอนเทรนกับตอนใช้งานไม่ตรงกันสองจุด จุดแรก ฝั่งเทรน (`regen_melspec.py` ของ repo server) คำนวณขาดไปหนึ่งเฟรม ทำให้คอลัมน์สุดท้ายค้างเป็นศูนย์แล้วไปกวนการปรับสเกลของทั้งภาพ จุดที่สอง บอร์ดอัดเสียงตามเลขที่กรอกในบล็อกแทนที่จะอัดตามความยาวที่โมเดลเทรนมา พอไม่ตรงกันเสียงจะถูกบีบตามแกนเวลาจนโมเดลอ่านไม่ออก ตอนนี้ `voice_runtime` ถามความยาวจากตัวโมเดลเองผ่าน `Classifier.input_width()` แล้วตัดช่วงที่ดังที่สุดให้พอดี วัดบนบอร์ดจริง: อัด 3 วินาทีกับโมเดลที่เทรนด้วยคลิป 1 วินาที เดิมได้ 33% (เท่าเดาสุ่มของงาน 3 คลาส) ตอนนี้ได้ 100%
- **ตัวเลื่อนตั้งระดับเสียงในหน้าเก็บข้อมูลไม่มีผล** ตั้งต่ำหรือสูงก็เริ่มอัดทันทีเหมือนกัน เพราะตัวเลื่อนใช้ช่วง 1-255 แต่บอร์ดวัดความดังด้วย `audioop.rms()` ที่มีช่วง 0-32768 ค่าสูงสุดที่ตั้งได้จึงเป็นเพียง 0.78% ของสเกลจริง ต่ำกว่าเสียงห้องทั่วไป ตอนนี้ตัวเลื่อนใช้หน่วยเดียวกับบอร์ด แสดงตัวเลขที่ตั้งไว้ และแสดงระดับเสียงสด ๆ ให้เทียบขณะปรับ แก้ทั้ง `AdbSoundCapture` (V1) และ `WsShellSoundCapture` (V2)

**Added**
- เพิ่มขาที่ใช้กับ Servo บนบอร์ด mAI Plus
