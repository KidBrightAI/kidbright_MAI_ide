export default {
  //========== yolo decoder ===========//
  yoloHead: function (feats, anchors, numClasses, inputShape) {
    ...
    const gridY = tf.tile(
      tf.reshape(tf.range(0, gridShape[0]), [-1, 1, 1, 1]),
      [1, gridShape[1], 1, 1],
    )
    const gridX = tf.tile(
      tf.reshape(tf.range(0, gridShape[1]), [1, -1, 1, 1]),
      [gridShape[0], 1, 1, 1],
    )
  ...
  yoloCorrectBoxes: function (boxXy, boxWh, imageShape) {
